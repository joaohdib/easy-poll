...
test('migration execution is idempotent and preserves existing data', () => {
  ...
  try {
    ...
    const group = connection.sqlite
      ...
      .get('group-kept');
    assert.equal(migrationCountBefore, 3);
...
test('adds vote snapshot availability over a Phase 7 database with safe defaults', () => {
  const connection = createDatabase(':memory:');
  try {
    applySqlMigration(connection.sqlite, '0000_sticky_karma.sql');
    applySqlMigration(connection.sqlite, '0001_breezy_kylun.sql');
    ...
    applySqlMigration(connection.sqlite, '0002_foamy_morg.sql');

    const poll = connection.sqlite.prepare(`
      SELECT question,
             votes_snapshot_available AS votesSnapshotAvailable,
             votes_snapshot_at AS votesSnapshotAt
      FROM polls WHERE message_id = ?
    `).get('legacy-poll');
    assert.deepEqual(poll, {
      question: 'Pergunta preservada',
      votesSnapshotAvailable: 0,
      votesSnapshotAt: null
    });
  } finally {
    connection.closeDatabase();
  }
