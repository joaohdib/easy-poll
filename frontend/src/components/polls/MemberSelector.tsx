...
export function MemberSelector({ groupId, hasFilledOptions, onApply, showToast }: MemberSelectorProps) {
  ...
  return <>
    <button className="button ghost" type="button" disabled={loading} onClick={() => void open()}><Users aria-hidden="true" />{loading ? 'Carregando membros…' : 'Selecionar membros'}</button>
