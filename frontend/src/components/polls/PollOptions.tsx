...
export function PollOptions({ children, options, onChange, showToast }: PollOptionsProps) {
  ...
  return <>
    <div className="field"><div className="label-row"><label>Opções</label><span>{options.length} de {MAX_POLL_OPTIONS}</span></div><div className="options-list">{options.map((option, index) => <div className="option-row" key={index}><span className="option-index" aria-hidden="true">{index + 1}</span><input className="poll-option" maxLength={100} placeholder={`Opção ${index + 1}`} aria-label={`Opção ${index + 1}`} required value={option} onChange={(event) => updateOption(index, event.target.value)} /><button className="remove-option" type="button" aria-label={`Remover opção ${index + 1}`} disabled={options.length <= 2} onClick={() => removeOption(index)}><Trash2 aria-hidden="true" /></button></div>)}</div><div className="option-actions"><button className="button ghost" type="button" disabled={options.length >= MAX_POLL_OPTIONS} onClick={addOption}><Plus aria-hidden="true" />Adicionar opção</button><button className="button ghost" type="button" onClick={openDialog}><ListPlus aria-hidden="true" />Colar várias opções</button>{children}</div></div>
