...
import { CalendarDays, Clock3, CloudRain, Feather, Flame, Handshake, Moon, PenLine, Rocket, Shuffle, Swords, Timer, Trophy, Vote, Zap } from 'lucide-react';
  ...
  return <div>
    ...
    <StatsSection step="Participação" title="Quem aparece em todas" note="Baseado apenas nos participantes identificados nas enquetes.">
      <div className="stat-card-grid two-columns">
        <StatsCard icon={<Trophy />} title="Mais participativo" name={stats.mostActive?.name} accent="winner" value={stats.mostActive ? formatPercent(stats.mostActive.participationRate) : ''} description={stats.mostActive ? `${stats.mostActive.pollsParticipated} de ${eligible} enquetes com dados de votos` : 'Nenhum participante foi identificado.'} />
        <StatsCard icon={<Moon />} title="Menos participativo" name={stats.leastActive?.name} value={stats.leastActive ? formatPercent(stats.leastActive.participationRate) : ''} description={stats.leastActive ? `${stats.leastActive.pollsParticipated} de ${eligible} enquetes com dados de votos` : 'Nenhum participante foi identificado.'} />
      </div>
    </StatsSection>
    ...
    <StatsSection step="Atividade" title="Quando o grupo mais vota" note="Horários convertidos para America/Sao_Paulo; os timestamps originais não são alterados.">
      <div className="stat-card-grid two-columns"><ActivityCard icon={<CalendarDays />} title="Dia mais ativo" name={stats.mostActiveDay ? capitalize(stats.mostActiveDay.name) : undefined} value={stats.mostActiveDay ? plural(stats.mostActiveDay.count, 'participação', 'participações') : ''} description={stats.mostActiveDay ? `${formatPercent(stats.mostActiveDay.percentage)} de toda a atividade com horário disponível.` : 'Nenhum voto possui timestamp válido disponível.'} items={stats.mostActiveDay?.distribution} /><ActivityCard icon={<Clock3 />} title="Horário nobre" name={stats.primeTime?.rangeLabel} value={stats.primeTime ? plural(stats.primeTime.count, 'participação', 'participações') : ''} description={stats.primeTime ? 'Faixa de uma hora com mais participações recuperadas.' : 'Nenhum voto possui timestamp válido disponível.'} items={stats.primeTime?.topHours} /></div>
    </StatsSection>
  </div>;
}
